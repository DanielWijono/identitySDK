...
```sh
swift test                                            # 65 tests, including live HTTP and lifecycle recovery
...
M0–M2 are implemented; M3 camera capture is code-complete and its remaining gates are physical; M4 has an HTTP adapter, an independent loopback demo server, live transport recovery coverage and a foreground lifecycle transport. This is not a v0.1 release. UIKit and SwiftUI simulation hosts are available. Image normalization and encrypted storage are integrated into the iOS sample hosts; see [normalization](Docs/Image-Normalization.md) and [vault contract and remaining gates](Docs/Evidence-Vault.md).
...
`HTTPVerificationProvider` implements the demo HTTP contract with bounded retry, reused evidence IDs and submission keys, and reconciliation before repeating any mutation, so a lost response cannot create a second logical submission. `ForegroundHTTPTransport` synchronously cancels active socket work on inactivity, sends nothing while backgrounded and converts an interrupted mutation into reconciliation after fresh activation. `DemoVerificationService` provides fast in-process semantic tests. A separate standard-library Python server implements the JSON contract independently and drives `URLSessionTransport` across a real loopback socket. The 65-test suite covers a complete live exchange, a commit whose connection is dropped after acceptance, and foreground lifecycle races. Optional local HTTPS uses supplied certificates and normal platform trust validation, but the automated suite does not install a test CA or claim TLS interoperability. See [demo server](Examples/DemoHTTPServer/README.md) and [provider contract](Docs/Provider-Contract.md).

M3 and M4 have no outstanding code deliverable. The repeated-camera-lifecycle gate passes on hardware: 19 device tests with zero skips on an iPhone 14 (iOS 17.3), with camera readiness p95 of 0.477 s and 0.385 s across two 30-start runs against a 1.5-second budget. What remains is hardening and release evidence: permission revocation and interruption checks on hardware, instrumented lock-state denial during crop editing, minimum-iOS hardware, hands-on VoiceOver, blur-threshold calibration, memory/thermal measurements, API documentation and release assets. Denied/restricted permission recovery and the largest Dynamic Type category are component-tested. See [assessment](Docs/Assessment.md), [state contract](Docs/State-Machine.md), and the [original plan](IdentityFlow-SDK-Plan.md).
