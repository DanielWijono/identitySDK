...
The full package suite passed all 62 tests. The two live tests completed in approximately 0.38 seconds after the server was changed to avoid an unnecessary reverse-DNS lookup during loopback binding. The automated tests use plain HTTP on `127.0.0.1`; they establish real socket behavior and independent JSON compatibility, but they do not install a test CA or claim TLS interoperability. At this checkpoint, foreground/background transfer reconciliation remained the open M4 lifecycle item.

## Foreground HTTP reconciliation — 20 September 2026

Added `ForegroundHTTPTransport`, a lock-backed lifecycle wrapper intended for `URLSessionTransport`. `leaveForeground()` closes the gate and cancels registered exchanges synchronously. Requests started while inactive wait without reaching the underlying transport. Fresh activation requires a generation-bound permit, preventing delayed work captured before inactivity from reopening network access.

When inactivity interrupts a GET, the wrapper repeats that read after activation. When it interrupts PUT or POST, it waits for activation and returns an ambiguous connection loss to `HTTPVerificationProvider`. The provider then executes its existing authoritative-state check before it can replay the mutation. This keeps lifecycle policy separate from HTTP idempotency while preserving both protections.

Three tests cover: zero underlying requests while inactive followed by successful fresh activation; caller cancellation returning without waiting for foreground; and a consent mutation applied by the server whose response is interrupted. The last case rejects a stale permit and records `PUT consent` followed by `GET state`, one consent write and no repeated PUT. The full package suite now passes 65 tests. `IdentityFlowHTTP` also builds directly for the generic iOS 16+ Simulator destination under Swift 6. The lifecycle tests use the in-process service; real `URLSession` socket behavior remains independently covered by the two loopback tests above. A production host/backend integration is still required before claiming field validation.
