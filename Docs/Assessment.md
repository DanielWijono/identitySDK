...
3. Continue M3: ImageNormalizer now handles bounded JPEG encoding and orientation/metadata removal; sample review/retake is implemented. The optional StillCamera and CameraReviewViewController are prepared but disconnected from the flow pending physical checks; cropping and the full SDK-owned facade remain. validate permissions, interruptions, accessibility and timing on physical hardware.
