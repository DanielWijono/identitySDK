...
- The sample hosts provide live front/back confirmation, retake, manual crop and encrypted handoff through the evidence-source boundary. Rectangle guidance and the Laplacian focus score are advisory and never block the shutter. The blur heuristic closes a v0.1 commitment that had been listed in the plan but never implemented or tracked; its threshold is calibrated only against synthetic fixtures. Denied/restricted permission recovery includes a Settings action and active-return recheck; physical revocation validation remains pending.
...
- A submission uses one UUID for its logical run, and `HTTPVerificationProvider` reuses it as the idempotency key. Bounded retry, pre-retry reconciliation and server-side duplicate protection are covered by mutation-verified in-process tests. An independent Python server now proves JSON compatibility and exercises `URLSessionTransport` over loopback, including a connection dropped after an accepted commit. Automated TLS interoperability remains untested; optional local HTTPS keeps normal platform trust validation.
- `ForegroundHTTPTransport` synchronously closes network access and cancels its current exchange on inactivity. It rejects stale activation permits, sends nothing while inactive, transparently resumes reads and reports interrupted mutations as ambiguous only after fresh activation. The provider must therefore reconcile authoritative state before a mutation can repeat. The sample still uses a local provider, so first-host integration with a real backend remains future evidence rather than an additional SDK code deliverable.
...
3. M3 implementation is complete and its automated hardware gate has passed: 19 device tests with zero skips on an iPhone 14, including 30 real camera start/stop cycles, duplicate-start rejection and both storage device tests. Camera readiness met the provisional p95 budget with roughly threefold margin on that device and build configuration. Remaining M3 items all need manual interaction: permission revocation, interruptions, hands-on VoiceOver and instrumented lock-state behavior.
4. M4 adapter, in-process demo service, independent HTTP server, live lost-response tests and foreground/background reconciliation transport are implemented.
