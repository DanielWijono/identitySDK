...
The optional camera/review components are preparatory and disconnected from the sample flow pending device validation. They keep pixels in memory and hand confirmed JPEGs to a host callback; they do not themselves encrypt, persist, or securely wipe those bytes. The host must connect vault ownership and lifecycle handling before real-document use. See Docs/Camera-Capture.md.
