...
- The core API remains headless. Independent UIKit and SwiftUI simulation apps consume the local package using a shared MainActor sample screen. The SDK now owns a single-side UIKit camera/review component plus a thin SwiftUI wrapper; a full front/back orchestration facade remains to be built.
...
3. Continue M3: ImageNormalizer, integrated live capture, manual crop, Vision rectangle guidance, UIKit camera/review and its SwiftUI wrapper are implemented. Permission recovery and largest-category Dynamic Type have automated coverage. Run the prepared 30-cycle physical lifecycle/timing test, then validate permission revocation, interruptions, hands-on VoiceOver and lock-state behavior. Full front/back SDK orchestration remains separate.
