...
The core terminates on an unrecovered provider failure and does not expose a second UI retry state. `HTTPVerificationProvider` performs bounded internal retry and authoritative reconciliation. Its optional `ForegroundHTTPTransport` cancels active network work on inactivity, blocks requests while inactive, transparently resumes reads, and forces an interrupted mutation back through reconciliation after fresh activation. Providers must not return an identity decision based merely on successful upload.
